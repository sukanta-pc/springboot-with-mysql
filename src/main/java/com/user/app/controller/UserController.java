...
public class UserController {
	...
	public @ResponseBody ResponseEntity<?> addNewUser (@ApiParam @RequestBody UserVO userVo) throws UserException {
		...
		if(!InputValidator.isValidateInput(userVo)){
			...
			throw new UserException("INPUT.DATA.INVALID","Input data not valid..");
		}
		User user = userService.saveUser(setUserData(userVo));
		AppLogger.getLogger().info("User Created successfully" + toStringInput(user));
		return new ResponseEntity<ResponseVO>(new ResponseVO(true,"New user Created successfully"),HttpStatus.CREATED);
	}

	
	@ApiOperation(value = "Update user", response = ResponseVO.class)
	@RequestMapping(method=RequestMethod.PUT) 
	public @ResponseBody ResponseEntity<?> updateUser (@ApiParam @RequestBody UserVO userVo) throws UserException {
		AppLogger.getLogger().info("input data: "+ toStringInput(userVo));
		//input validation
		...
		User user = userService.saveUser(setUserData(userVo));
		AppLogger.getLogger().info("User data updated successfully.." + toStringInput(user));
		return new ResponseEntity<ResponseVO>(new ResponseVO(true,"User data updated successfully"),HttpStatus.OK);
	}

	@ApiOperation(value = "Delete user", response = ResponseVO.class)
	@RequestMapping(path="/{id}", method=RequestMethod.DELETE)
	public @ResponseBody ResponseEntity<?> deleteUser (@ApiParam @PathVariable Long id) throws UserException {
		//input validation
		if(isValidId(id)){
			AppLogger.getLogger().info("User Id Invalid..");
			throw new UserException("USER.ID.INVALID","User to delete doesn´t exist");
		}
		User n = new User();
		n.setId(id);
